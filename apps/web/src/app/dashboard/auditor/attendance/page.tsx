...
import { TrendingUp, AlertTriangle, Users, ShieldCheck } from 'lucide-react';
...
export default function AuditorAttendancePage() {
  ...
  const columns = [
    ...
    { key: 'total', header: 'Total Sessions' },
    ...
    <div>
      ...
      <div className="p-6 space-y-6">
        <div className="bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 rounded-xl p-4 flex items-center gap-3">
          ...
          <p className="text-sm text-blue-800 dark:text-blue-300">Read-only audit view.</p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <StatCard title="Below 80%" value={low.length} icon={AlertTriangle} color="red" />
        </div>

        <div className="card p-6">
          <PageHeader title="Students Below 80% Attendance" />
          <DataTable columns={columns} data={low} emptyMessage="All students are above the 80% attendance threshold." />
        </div>
      </div>
    </div>
  );
